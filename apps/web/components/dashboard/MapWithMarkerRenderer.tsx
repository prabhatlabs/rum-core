...
function getCountryData(rows: unknown[], countryCol: string): CountryData[] {
    ...
    for (const row of rows) {
        ...
        if (code && COUNTRY_COORDS[code]) {
            const total =
                Number(
                    record["total_requests"] ?? record["session_count"] ?? 0,
                ) || 0;
...
export function MapWithMarkerRenderer({
    ...
    const countryOpacities = React.useMemo(() => {
        ...
        for (const item of countryData) {
            map.set(
                item.code,
                Math.round(10 + (item.totalRequests / maxRequests) * 70),
            );
        }
        return map;
                    ...
                    <ZoomableGroup center={[0, -40]} zoom={1}>
                        <Geographies geography={TOPO_URL}>
                            {({ geographies }) =>
                                geographies.map((geo) => {
                                    const alpha2 =
                                        ISO_NUM_TO_ALPHA2[geo.id] ?? "";
                                    const opacity =
                                        countryOpacities.get(alpha2);
                ...
                {tooltip && (
                    ...
                    >
                        <div className="flex items-center gap-1.5 bg-foreground text-background text-xs px-2 py-1 whitespace-nowrap">
                            ...
                            <span>
                                {tooltip.totalRequests.toLocaleString()}{" "}
                                requests
                            </span>
                        </div>
