...
export async function incrementUsage(projectKey: string, calls: number) {
    if (calls <= 0) {
        throw new Error(
            `incrementUsage called with invalid calls value: ${calls}`,
        );
    }
...
export async function getCallsLeft(
    projectKey: string,
): Promise<{ remaining: number; limit: number; used: number; origin: string }> {
    const currentDate = getCurrentDate();
    ...
    const planType = result[0]
        .plan_type as keyof typeof constants.plans.PLAN_LIMITS;
...
export async function getCallsLeftHttp(
    projectKey: string,
): Promise<{ remaining: number; limit: number; used: number; origin: string }> {
    const currentDate = getCurrentDate();
    const db = getMainDBHttp();
...
export async function cleanupUsage(): Promise<void> {
    const cutoff92Days = getCutoffTimestamp(
        constants.plans.RETENTION.usage_max_days,
    );
...
export async function getUserUsageByDate(
    userId: string,
    timeRange: "today" | "7d" | "30d" | "60d",
): Promise<UsageByDate[]> {
    const db = getMainDB();
