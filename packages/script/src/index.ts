...
(function (global: any) {
    ...
    try {
        ...
        const PROJECT_KEY =
            (document.currentScript as HTMLScriptElement | null)?.getAttribute(
                "data-key",
            ) ?? null;
        const WORKER_URL =
            (document.currentScript as HTMLScriptElement | null)?.getAttribute(
        ...
        if (!WORKER_URL) {
            console.warn(
                "[rum-core] Missing data-worker attribute on script tag",
            );
            return;
        }
        ...
        function getEnvironment() {
            ...
            let browser = "Unknown",
                browser_version: string | null = null;
            if (/Edg\//.test(ua)) {
                browser = "Edge";
                ...
                browser = "Chrome";
                browser_version =
                    (ua.match(/Chrome\/([\d.]+)/) || [])[1] || null;
            } else if (/Firefox\//.test(ua)) {
                browser = "Firefox";
                browser_version =
                    (ua.match(/Firefox\/([\d.]+)/) || [])[1] || null;
            ...
            if (/Windows NT/.test(ua)) {
                ...
                os = "macOS";
                os_version =
                    ((ua.match(/Mac OS X ([\d_]+)/) || [])[1] || "").replace(
                        /_/g,
                        ".",
                    ) || null;
        ...
        function normalizeUrl(rawUrl: string): string {
            try {
                ...
                const normalizedPath = url.pathname
                    .split("/")
                    .map((seg) =>
                        /^[0-9a-f-]{8,}$|^\d+$/.test(seg) && seg.length > 0
                            ? ":id"
                            : seg,
                    )
                    .join("/");
        ...
        function flushEvents() {
            ...
            sendData(
                EVENTS_URL,
                JSON.stringify({ events: batch, project_key: PROJECT_KEY }),
            );
        }
        ...
        function computeVitalsScore(
            ...
            function rate(v: number, good: number, ni: number): Rating {
                return v <= good ? "good" : v <= ni ? "ni" : "poor";
            }
            const scores: number[] = [];
        ...
        function initWebVitals() {
            try {
                ...
                function trySend() {
                    if (!sent && c.fcp !== null && c.lcp !== null) {
                        sent = true;
                        sendVitals(c.lcp, c.fcp, c.cls, c.inp);
                    }
                }
        ...
        global.fetch = async function (
            ...
            if (body) {
                if (typeof body === "string")
                    ...
                    try {
                        requestSize = (
                            await new Request("https://temp.com", {
                                method: "POST",
                                body,
                            })
                                .clone()
                ...
                .then((response: Response) => {
                    ...
                    eventQueue.push(
                        buildRequestEvent({
                            ...
                            ...timing,
                            duration:
                                timing.duration ?? performance.now() - start,
                        }),
                    );
                    return response;
                })
        ...
        function createXHRPatched(this: XMLHttpRequest) {
            ...
            self.getResponseHeader = function (h: string) {
