...
export const users = pgTable(
    ...
    (t) => [unique().on(t.provider, t.provider_id)],
);
...
export const usage = pgTable(
    ...
    (t) => [
        ...
        index("idx_usage_user_project_date").on(
            t.user_id,
            t.project_key,
            t.date,
        ),
...
export const monthly_usage = pgTable(
    "monthly_usage",
    {
        id: uuid("id").primaryKey().defaultRandom(),
        user_id: uuid("user_id")
        ...
        calls_million: decimal("calls_million", { precision: 10, scale: 4 })
            .notNull()
            .default("0"),
        created_at: timestamp("created_at").defaultNow().notNull(),
    },
